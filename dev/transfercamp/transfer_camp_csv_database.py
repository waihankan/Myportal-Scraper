...
for csv_file in csv_files:
   ...
   for row in df.itertuples():
      insert_transfer_camp(getattr(row, "Year"), getattr(row, "Semester"), getattr(row, "Instructor"), getattr(row, "Subj"), getattr(row, "Crse"), getattr(row, "A"), getattr(row, "B"), getattr(row, "C"), getattr(row, "D"), getattr(row, "F"), getattr(row, "W"))
